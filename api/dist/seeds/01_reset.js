...
exports.seed = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (knex) {
    console.log('Borrando datos...');

    try {
      yield Promise.all([// Eliminar todos los registros
      knex('productos').del(), knex('categorias').del(), knex('clientes').del(), knex('pedidos').del(), knex('producto_pedido').del(), knex('usuarios').del(), knex('tipo_pago').del(), knex('info_bancaria').del(), knex('pago_movil').del(), knex('empresas').del(), knex('empresa_info_bancaria').del(), knex('empresa_pago_movil').del(), knex('ivas').del(), knex('roles').del(), // Categorias por defecto
      ...
      knex('empresa_info_bancaria').insert({
        id: 1,
        id_empresa: 1,
        id_info_bancaria: 1
      }), // fin
      // Relación empresa-pago_movil por defecto (una a muchos)
      knex('empresa_pago_movil').insert({
        id: 1,
        id_empresa: 1,
        id_pago_movil: 1
      }), // fin
      // Empresa por defecto
      knex('empresas').insert({
        id: 1,
      ...
      console.log("Ocurri\xF3 un error: ".concat(error));
    }
  });
